...
class GINE(nn.Module):
    def __init__(self, *,
                 ...
                 edge_dim=None, node_encoder=None, edge_encoder=None, num_trans_layers=1, eps=0.0, train_eps=False,
        ...
        for in_channels, out_channels in zip(channel_list[:-1], channel_list[1:]):
            self.convs.append(GINEConv(
                nn=MLP(in_channels=in_channels, hidden_channels=out_channels,
                       out_channels=out_channels, num_layers=num_trans_layers),
                eps=eps, train_eps=train_eps, edge_dim=edge_dim, **kwargs))
            if batch_norm:
