...
from torch_geometric.nn.conv import EGConv
...
class EGC(nn.Module):
    def __init__(self, *,
                 ...
                 edge_dim=None, node_encoder=None, edge_encoder=None, use_multi_aggregators=True,
                 num_heads=8, num_bases=4, num_pred_heads=None, num_pred_layers=3, readout=None,
        ...
        for in_channels, out_channels in zip(channel_list[:-1], channel_list[1:]):
            self.convs.append(EGConv(in_channels=in_channels, out_channels=out_channels,
                                     aggregators=aggregators, num_heads=num_heads, num_bases=num_bases, **kwargs))
            if batch_norm:
