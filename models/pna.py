...
class PNA(nn.Module):
    def __init__(self, *,
                 ...
                 edge_dim=None, node_encoder=None, edge_encoder=None, aggregators=['mean', 'min', 'max', 'std'],
                 scalers=['identity', 'amplification', 'attenuation'], deg=None, towers=1, divide_input=False,
                 num_pre_layers=1, num_post_layers=1, num_pred_heads=None, num_pred_layers=3, readout=None,
                 dropout=0.0, batch_norm=True, act='relu', act_first=False, act_kwargs=None, **kwargs):
        ...
        for in_channels, out_channels in zip(channel_list[:-1], channel_list[1:]):
            self.convs.append(PNAConv(in_channels=in_channels, out_channels=out_channels, edge_dim=edge_dim,
                                      aggregators=aggregators, scalers=scalers, deg=deg,
                                      pre_layers=num_pre_layers, post_layers=num_post_layers,
                                      towers=towers, divide_input=divide_input, **kwargs))
            if batch_norm:
